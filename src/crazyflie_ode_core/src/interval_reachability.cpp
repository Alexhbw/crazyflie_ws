...
// ------------------------------------------------------------
// 多步可达管: 从 x0 开始, 重复计算 k 步区间传播
// 每步: 以上一步区间的中心为起点, ±ε 扰动, 检测碰撞
// ------------------------------------------------------------
std::vector<IntervalReachability::TubeStep>
IntervalReachability::computeReachTube(int k, SolverFn solver_fn,
                                       const Vector12d& x0,
                                       const Vector4d& u, double h)
{
    std::vector<TubeStep> tube;
    Vector12d x_current = x0;

    for (int step = 1; step <= k; ++step) {
        // 一步区间传播
        IntervalBounds bounds = computeStepInterval(solver_fn, x_current, u, h);

        // 碰撞检测
        double min_d;
        bool overlap = checkObstacleOverlap(bounds, min_d);

        TubeStep ts;
        ts.step = step;
        ts.bounds = bounds;
        ts.min_obstacle_dist = min_d;
        ts.overlap = overlap;
        tube.push_back(ts);

        // 如果已经碰撞, 后续步骤用最后一次边界推算
        if (overlap) break;

        // 用区间中心作为下一步起点 (名义轨迹)
        x_current = bounds.center;
    }
